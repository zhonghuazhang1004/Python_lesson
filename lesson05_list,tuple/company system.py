...
while system_running:
    ...
    # Perform necessary calculations and update the account and warehouse accordingly.
        ...
        if sale_name in inventory.keys():
            ...
            if sale_price > 0:
                ...
                if product_stock <= 0:
                    print(f"***Warning: no enough stock for {sale_name}---current stock: {product_stock}***\n")
    ...
    # Ensure that the account balance is not negative after a purchase operation.
        ...
        if purchase_name in inventory.keys():
            ...
            if sale_price > 0:
                ...
                inventory[sale_name]["stock"] = product_stock + purchase_quantity
    ...
    # Handle cases where 'from' and 'to' values are out of range.
        ...
        print("------Review------")
        from_index = int(input("Enter 'from' indices: ")) - 1
        to_index = int(input("Enter 'to' indices: ")) - 1
        if from_index < 0 or to_index >= len(operation_collection) or from_index > to_index:
